...
- [x] `classify()` のユニットテスト 5 件追加 (AC / CE / 非数値 cpuTime / TLE / WA)
- [ ] 手元 `docker compose` で実 JDoodle 呼び出しを含む AC / CE を 1 件ずつ確認
- [ ] dev へ merge 後 staging で同じ検証
- [x] `cargo test` (8 passed) / `cargo clippy --all-targets -- -D warnings` (warning なし)
...
- 2026-06-17 (続): 判定ロジックを `classify()` 純粋関数に抽出。fixture (`migration/jdoodle-response-{success,ce}.json`) を
  `include_str!` で読み込むユニットテストを 5 件追加 (AC / CE 判定順序保証 / 非数値 cpuTime panic 回避 / TLE / WA)。
  `cargo test` 8 passed、`cargo clippy --all-targets -- -D warnings` clean。残りは手元 compose 検証と staging 実機。
