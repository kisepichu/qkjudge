...
- **backup**: `mariadb-dump` (mysqldump 互換、mariadb:11.4 同梱) CronJob → PVC (将来 R2/B2 等のオフサイトに拡張)。リストア手順も README に。
...
### backup
- [x] `mariadb-dump` CronJob + リストア手順 (`backup-cronjob.yaml` + README)
### 整合
...
- 2026-06-02: `deploy/k3s/` マニフェスト + README を整備。
  ...
  - **app**: 非 secret env は `qkjudge-config` (configMap) を envFrom、secret はアプリが読む 5 キーのみ
    `secretKeyRef` で個別注入 (最小権限。`MARIADB_ROOT_PASSWORD` をアプリに入れない)。可変タグを確実に
    pull するため `imagePullPolicy: Always`。`MARIADB_USER`(DB)/`MARIADB_USERNAME`(app) の名前不一致は
    両キーを同値で持たせた。`/ping` を probe に使用。
